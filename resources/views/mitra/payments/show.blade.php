    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="mb-6">
            ...
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-3xl font-bold text-gray-900">Detail Pembayaran
                        #{{ str_pad($payment->id, 6, '0', STR_PAD_LEFT) }}</h1>
                ...
                <div class="flex items-center space-x-3">
                    @if(in_array($payment->status, ['verified', 'settlement', 'capture']))
                        <button type="button" data-payment-id="{{ $payment->id }}"
                            class="download-receipt inline-flex items-center px-4 py-2 bg-gradient-to-r from-orange-500 to-orange-600 text-white rounded-xl hover:from-orange-600 hover:to-orange-700 transition shadow-lg">
                            <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    @if($payment->status === 'pending')
                        <button type="button" data-payment-id="{{ $payment->id }}"
                            class="verify-payment inline-flex items-center px-4 py-2 bg-green-600 text-white rounded-xl hover:bg-green-700 transition shadow-lg">
                            <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        ...
                        <button type="button" data-payment-id="{{ $payment->id }}"
                            class="reject-payment inline-flex items-center px-4 py-2 bg-red-600 text-white rounded-xl hover:bg-red-700 transition shadow-lg">
                            <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <div class="bg-white rounded-2xl shadow-lg p-6 border border-gray-100">
                    ...
                    <div class="space-y-4">
                        <div class="flex items-center justify-between py-3 border-b border-gray-100">
                            ...
                            <span
                                class="text-base font-semibold text-gray-900">#{{ str_pad($payment->booking->id, 6, '0', STR_PAD_LEFT) }}</span>
                        </div>
                        <div class="flex items-center justify-between py-3 border-b border-gray-100">
                        ...
                        <div class="flex items-center justify-between py-3 border-b border-gray-100">
                            ...
                            <span
                                class="text-base font-semibold text-gray-900">{{ $payment->booking->user->name ?? $payment->booking->tenant_name }}</span>
                        </div>
                        <div class="flex items-center justify-between py-3 border-b border-gray-100">
                        ...
                        <div class="flex items-center justify-between py-3 border-b border-gray-100">
                            ...
                            <span
                                class="text-base text-gray-900">{{ \Carbon\Carbon::parse($payment->booking->check_in_date)->format('d F Y') }}</span>
                        </div>
                        <div class="flex items-center justify-between py-3 border-b border-gray-100">
                            <span class="text-sm font-medium text-gray-500">Check Out</span>
                            <span
                                class="text-base text-gray-900">{{ \Carbon\Carbon::parse($payment->booking->check_out_date)->format('d F Y') }}</span>
                        </div>
                        <div class="flex items-center justify-between py-3">
                            ...
                            <span class="text-lg font-bold text-orange-600">Rp
                                {{ number_format($payment->booking->final_amount, 0, ',', '.') }}</span>
                        </div>
                    </div>
                    <div class="mt-6">
            ...
            <div class="space-y-6">
                        ...
                        @if($payment->proof_image)
                            <div class="mb-4">
                                <div class="relative group cursor-pointer"
                                    onclick="showImageModal('{{ Storage::url($payment->proof_image) }}')">
                            ...
                            <a href="{{ route('mitra.payments.download-proof', $payment) }}" target="_blank"
                ...
                @if($payment->status === 'pending')
                    <div class="bg-gradient-to-br from-orange-50 to-orange-100 rounded-2xl p-6 border border-orange-200">
                        ...
                        <div class="space-y-3">
                            <button type="button" data-payment-id="{{ $payment->id }}"
                                class="verify-payment w-full px-4 py-3 bg-green-600 text-white rounded-xl hover:bg-green-700 transition font-medium shadow-lg flex items-center justify-center">
                                <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                                </svg>
                            ...
                            <button type="button" data-payment-id="{{ $payment->id }}"
                                class="reject-payment w-full px-4 py-3 bg-white text-red-600 border-2 border-red-600 rounded-xl hover:bg-red-50 transition font-medium flex items-center justify-center">
                                <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                                </svg>
