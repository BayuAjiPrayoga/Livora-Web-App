...
@section('title', 'Edit ' . $room->name . ' - ' . $property->name . ' - LIVORA')
...
<div class="bg-livora-background min-h-screen">
    ...
    <div class="bg-white shadow-sm border-b">
        <div class="px-6 py-4">
            <div class="flex items-center space-x-2 text-sm text-gray-600 mb-2">
                ...
                <a href="{{ route('mitra.rooms.show', ['property' => $property->id, 'room' => $room->id]) }}" class="hover:text-livora-primary">{{ $room->name }}</a>
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path>
                </svg>
                <span class="text-livora-primary font-medium">Edit Kamar</span>
            </div>
            <div class="flex justify-between items-center">
                <div>
                    <h1 class="text-2xl font-bold text-livora-text">Edit Kamar</h1>
                    <p class="text-gray-600 mt-1">Update informasi kamar {{ $room->name }}</p>
                </div>
    ...
    <div class="p-6">
        ...
        <form action="{{ route('mitra.rooms.update', ['property' => $property->id, 'room' => $room->id]) }}" method="POST" enctype="multipart/form-data" class="max-w-4xl">
            @csrf
            @method('PUT')
            ...
            <div class="bg-white rounded-lg shadow-md">
                ...
                <div class="px-6 py-4 border-b border-gray-200">
                    ...
                    <p class="text-gray-600 text-sm mt-1">Update detail kamar sesuai kebutuhan</p>
                </div>
                ...
                <div class="p-6 space-y-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <select name="is_available" id="is_available" 
                                ...
                                <option value="0" {{ old('is_available', $room->is_available) == 0 ? 'selected' : '' }}>Terisi</option>
                            </select>
                            @error('is_available')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                    ...
                    <div>
                        ...
                        <textarea name="description" id="description" rows="4" 
                                  ...
                                  placeholder="Deskripsikan fasilitas kamar, kondisi, dan keunggulannya...">{{ old('description', $room->description) }}</textarea>
                        @error('description')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                ...
                <div class="px-6 py-4 border-t border-b border-gray-200 bg-gray-50">
                    ...
                    <p class="text-gray-600 text-sm mt-1">Update foto kamar atau tambah foto baru</p>
                </div>
                    ...
                    <div class="mb-6">
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                            ...
                            <div class="relative group">
                                <img src="{{ Storage::url($image) }}" alt="Room Image" 
