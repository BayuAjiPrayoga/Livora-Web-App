...
class BookingResource extends JsonResource
{
    ...
    public function toArray(Request $request): array
    {
        return [
            ...
            'start_date' => $this->check_in_date?->format('Y-m-d'),
            'start_date_formatted' => $this->check_in_date?->format('d M Y'),
            'end_date' => $this->check_out_date?->format('Y-m-d'),
            'end_date_formatted' => $this->check_out_date?->format('d M Y'),
            'duration' => $this->duration_months,
            'duration_text' => $this->duration_months . ' bulan',
            'total_price' => (float) $this->total_amount,
            'total_price_formatted' => 'Rp ' . number_format($this->total_amount, 0, ',', '.'),
            'final_amount' => $this->final_amount ? (float) $this->final_amount : (float) $this->total_amount,
            'final_amount_formatted' => 'Rp ' . number_format($this->final_amount ?? $this->total_amount, 0, ',', '.'),
            'status' => $this->status,
